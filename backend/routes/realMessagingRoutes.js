...
module.exports = (pool) => {
  ...
  router.get('/stats/:userType/:userId', async (req, res) => {
    try {
      ...
      // Si userId est un email (pour les utilisateurs RH), convertir en ID
      let actualUserId = userId;
      if (userType === 'rh' && userId.includes('@')) {
        try {
          const userQuery = await pool.query(
            'SELECT id FROM users WHERE email = $1',
            [userId]
          ...
          if (userQuery.rows.length > 0) {
            ...
            // Si l'utilisateur n'existe pas, retourner des statistiques vides
      ...
      const result = await pool.query(statsQuery, [actualUserId, userType]);
      ...
      const recentResult = await pool.query(recentQuery, [actualUserId, userType]);
