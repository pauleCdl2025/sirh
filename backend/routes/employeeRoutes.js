...
const decodeHtmlEntities = (str) => {
  ...
  const specificCorrections = [
    ...
    { wrong: 'g,n,rale', correct: 'générale' },
    { wrong: 'Direction G,n,rale', correct: 'Direction Générale' },
    { wrong: 'Direction g,n,rale', correct: 'Direction générale' },
  ...
  // Corriger les caractères Unicode problématiques (‚ U+201A remplace souvent é)
  // Remplacer ‚ par é dans les mots français courants
  decoded = decoded.replace(/G‚n‚rale/g, 'Générale');
  decoded = decoded.replace(/g‚n‚rale/g, 'générale');
  decoded = decoded.replace(/Direction G‚n‚rale/g, 'Direction Générale');
  decoded = decoded.replace(/Direction g‚n‚rale/g, 'Direction générale');
  // Correction générique : remplacer ‚ par é quand il est entre deux lettres (avant les autres corrections)
  decoded = decoded.replace(/([A-Za-z])‚([a-z])/g, '$1é$2');
  decoded = decoded.replace(/([A-Za-z])‚([A-Z])/g, '$1é$2');
  // Corriger aussi les virgules normales
  decoded = decoded.replace(/G,n,rale/g, 'Générale');
  decoded = decoded.replace(/g,n,rale/g, 'générale');
  decoded = decoded.replace(/Direction G,n,rale/g, 'Direction Générale');
  decoded = decoded.replace(/Direction g,n,rale/g, 'Direction générale');
...
const decodeEmployeeFields = (employee) => {
  ...
  return {
    ...
    // Statut marital
    statut_marital: decodeHtmlEntities(employee.statut_marital),
...
const fixEncodingInDatabase = async (pool) => {
  try {
    // Liste des corrections à effectuer pour les deux tables
    const corrections = [
      // Corrections pour functional_area (Direction) - avec caractères Unicode
      ...
      { table: 'effectif', column: 'functional_area', wrong: 'Direction g‚n‚rale', correct: 'Direction générale' },
      { table: 'employees', column: 'functional_area', wrong: 'Direction g‚n‚rale', correct: 'Direction générale' },
      // Corrections pour les virgules normales aussi
      { table: 'effectif', column: 'functional_area', wrong: 'G,n,rale', correct: 'Générale' },
      { table: 'employees', column: 'functional_area', wrong: 'G,n,rale', correct: 'Générale' },
      { table: 'effectif', column: 'functional_area', wrong: 'g,n,rale', correct: 'générale' },
      { table: 'employees', column: 'functional_area', wrong: 'g,n,rale', correct: 'générale' },
    ...
    for (const correction of corrections) {
      try {
        ...
        if (result.rowCount > 0) {
          console.log(`✅ ${correction.table}.${correction.column}: ${result.rowCount} ligne(s) corrigée(s)`);
        }
      } catch (error) {
